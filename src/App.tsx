...
type ViewState = "landing" | "create_account" | "login" | "forgot_password" | "feed" | "admin_login" | "admin_dashboard" | "profile";
...
export default function App() {
  ...
  const [currentUserData, setCurrentUserData] = useState<{ id: string; username: string; displayName: string; photoURL: string } | null>(() => {
    const saved = localStorage.getItem("bubble_current_user_data");
  ...
  const handleCreateBubble = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!username || !displayName || !password || !bio) return;

    const finalPhotoUrl = photoDataUrl || `https://picsum.photos/seed/${username}/600/800`;
    ...
    try {
      ...
      localStorage.setItem("bubble_current_user", displayName);
      localStorage.setItem("bubble_current_user_data", JSON.stringify(res.user));
      setView("feed");
    } catch (error: any) {
      alert(error.message);
    }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCurrentUserData(res.user);
      localStorage.setItem("bubble_current_user", res.user.displayName);
      localStorage.setItem("bubble_current_user_data", JSON.stringify(res.user));
      setView("feed");
    } catch (error: any) {
      alert(error.message);
  ...
  if (view === "forgot_password") {
    const handleCheckStatus = async (e: React.FormEvent) => {
      e.preventDefault();
      try {
        const res = await db.getPasswordResetStatus(forgotUsername);
        setResetStatus(res.status);
        if (res.status === "none") {
          await db.requestPasswordReset(forgotUsername);
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
      e.preventDefault();
      try {
        await db.resetPassword({ username: forgotUsername, newPassword });
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6 font-sans transition-colors">
        ...
        <div className="max-w-2xl mx-auto mt-20">
          <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col items-center mb-8">
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-2">@{currentUserData?.username}</p>
            <button
              onClick={() => {
                setCurrentUser("Anonymous");
                setCurrentUserData(null);
                localStorage.removeItem("bubble_current_user");
                localStorage.removeItem("bubble_current_user_data");
                setView("landing");
              }}
              className="mt-6 px-6 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors font-medium text-sm"
            >
              Log Out
            </button>
          </div>
                ...
                <tbody>
                  {adminUsers.map((user) => (
                    <tr key={user.id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                      ...
                      <td className="p-4 text-sm text-gray-900 dark:text-gray-100 font-medium">{user.username}</td>
                ...
                <tbody>
                  {passwordRequests.map((req) => (
                    <tr key={req.id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                      <td className="p-4 text-sm text-gray-900 dark:text-gray-100 font-medium">{req.username}</td>
                      <td className="p-4 text-sm">
